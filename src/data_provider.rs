...
pub mod imp {
    ...
    pub struct DataProvider {
        ...
        task_runner: Rc<RefCell<Option<TaskSender>>>,
    }
        ...
        pub fn establish_connection(&self) {
            ...
            let joinhandle = thread::spawn(move || {
                Builder::new_current_thread()
                    ...
                    .block_on(async move {
                        ...
                        let result = {
                            let client = admin_client.read();
                            tokio::select! {
                                _ = tokio::time::sleep(timeout_duration) => {
                                    println!("Watch call timeout");
                                    return;
                                },
                                result = client.as_ref().unwrap().watch() => match result {
                                    Ok(result) => result,
                                    Err(e) => {
            ...
            *self.task_runner.borrow_mut() = Some(task_tx);
            ...
            glib::spawn_future_local(async move {
                let Ok((channel, initial)) = event_rx.recv().await else {
                    task_runner.take();
                    return;
                };
                println!("Got stuff!");
